...
# 4.1. - Modulación 16-QAM
# Utilizando el código proporcionado por la teoría, se modifica para
# el caso de la Modulación 16-QAM
import numpy as np
...
def modulador(bits, fc, mpp):
    ...
    # Se procede a establece el contador de muestreos j
    ...
    for i in range(0, N, 4):
        ''' Se procede a definir cada uno de los bits, según los valores para el 16-QAM:
        b1 = bits[i]; b2 = bits[i+1];
        b3 = bits[i+2]; b4 = bits[i+3]
        Basicamente se toman en cuenta el caso para cada uno de los bits y la ubicación 
        que tienen estos dentro del mapa de la constelación'''
        
        senal_Tx[j*mpp: (j+1)*mpp] =             (-1)**(1+bits[i]) * 3**(1-bits[i+1]) * portadora1             + (-1)**(bits[i+2]) * 3**(1-bits[i+3]) * portadora2
        j += 1
...
def demodulador(senal_Rx, portadora1, portadora2, mpp):
    ...
    # Demodulación
    for i in range(N):
        #Se tiene que para esta función se iguala la cantidad de iteraciones que puede correr el programa y la cantidad de información que se puede recuperar
        if j+4 > N:  
            break   
        ...
        #Se tiene que asignar el signo asociado a este punto
        if np.sum(producto1) >= 0:
            bits_Rx[j] = 1  # Para el bit 1
        # Hay que tener mucho cuidado con el signo asociado ya que es negativo
        if np.sum(producto2) < 0:
            bits_Rx[j+2] = 1  # Para el bit 3

        # Ahora se procede a calcular la magnitud 
        if np.max(np.abs(producto1)) < 3:
            bits_Rx[j+1] = 1  # Para el bit 2
        if np.max(np.abs(producto2)) < 3:
...
A = [-3,-1,1,3]              # Se tiene cuatro posibles valores para la amplitud
N = 16                       # Hay 16 posibilidades de la onda
H_t = np.empty((N, len(t)))  # N funciones del tiempo x(t) con T puntos
J_t = np.empty((N, len(t)))
...
for i in A:
    ...
    Z = -i*np.cos(2*np.pi*fc*t) - i*np.sin(2*np.pi*fc*t)
...
Rea, = plt.plot(t, P, lw=6, color = 'green', label='Promedio realizaciones')
...
for n in range(N):
    for i, tau in enumerate(desplazamiento):
        corr[n, i] = np.correlate(H_t[n,:], np.roll(H_t[n,:], tau))/T
